...
const StepCategory = ({ formData, updateFormData, onNext }) => {
    ...
    if (categories?.length === 0) {
        return (
            <div className="py-16 text-center">
                <p className="text-gray-500 dark:text-gray-400">No product categories available</p>
            </div>
        );
    }
    ...
    return (
        <div>
            <h2 className="text-lg font-semibold text-gray-950 dark:text-white">What are you selling?</h2>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Choose the category that best fits your item</p>
                ...
                {categories?.map((category) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex flex-col items-center gap-3 p-4 rounded-2xl border-2 transition-all duration-200 ${isSelected
                                    ? 'border-gray-950 bg-gray-50 dark:border-blue-500 dark:bg-blue-950/20'
                                    : 'border-gray-200 hover:border-gray-300 dark:border-slate-700 dark:hover:border-slate-600'
                                }`}
                        >
                            <div
                                className={`flex items-center justify-center w-12 h-12 rounded-xl ${isSelected
                            ...
                            <span className="text-xs text-center text-gray-500 dark:text-gray-400">
                                {category.module?.toLowerCase() || 'Product'}
                            </span>
                        </button>
                    );
                })}
